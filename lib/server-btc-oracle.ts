...
export type OracleObservation = { source: string; price: number };
...
async function coinbase(): Promise<OracleObservation> {
...
async function kraken(): Promise<OracleObservation> {
...
async function gemini(): Promise<OracleObservation> {
...
export async function fetchBtcUsdOracleRound(): Promise<OracleRound> {
  ...
  const failures = settled.flatMap((result, index) => result.status === "rejected"
    ...
    : []);
  if (observations.length < 2) throw new Error("BTC/USD oracle quorum unavailable");
  ...
  if (maxDeviationBps > MAX_DEVIATION_BPS) throw new Error(`BTC/USD oracle sources disagree (${maxDeviationBps.toFixed(2)} bps)`);
  ...
  const roundId = createHash("sha256")
    ...
    .digest("hex");
  return { market: "BTC-USD", price, sourceCount: observations.length, maxDeviationBps, observations, failures, generatedAt, roundId };
}
